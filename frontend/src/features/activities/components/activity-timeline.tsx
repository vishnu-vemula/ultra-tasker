...
export function ActivityTimeline({ activities }: ActivityTimelineProps) {
  ...
  return (
    <ol className="relative space-y-6 before:absolute before:bottom-3 before:left-[1.125rem] before:top-3 before:w-px before:bg-slate-200">
      {activities.map((activity) => {
        const Icon = typeIcons[activity.type]
        return (
          <li key={activity.id} className="relative flex gap-4">
            <span className="z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-50">
              <Icon className="h-4 w-4 text-indigo-600" />
            </span>
            <div className="min-w-0 pt-1">
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-sm font-medium text-slate-900">{activity.title}</p>
                <span className="badge bg-slate-100 text-slate-600">{titleCase(activity.type)}</span>
              </div>
              ...
              {activity.body ? (
                <p className="mt-1 whitespace-pre-line text-sm text-slate-600">{activity.body}</p>
              ) : null}
            </div>
          </li>
        )
